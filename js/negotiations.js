  ...
  try {
    ...
    container.innerHTML = negotiations.map(neg => `
      <div class="card negotiation-card" data-id="${neg._id}">
        <h3>Lodge: ${escapeHtml(neg.lodge.name)}</h3>
...
async function showNegotiationDetail(id) {
  try {
    ...
    container.innerHTML = `
      <p><strong>Lodge:</strong> ${escapeHtml(negotiation.lodge.name)}</p>
      ...
      <div id="neg-messages">
        ${negotiation.messages.map(msg => `
          <div><strong>${escapeHtml(msg.sender.name)}:</strong> ${escapeHtml(msg.content)}</div>
        `).join('')}
      </div>
    `;
...
module.exports = { initNegotiations };
